...
function getOrCreateSlot(entry, wsp, parentEl) {
  ...
  if (entry.type === 'browser') {
    ...
    // iframe for proxied pages — provides complete CSS isolation
    ...
    const proxyUrl = u => `http://${location.hostname}:${PROXY_PORT}/proxy?url=${encodeURIComponent(u)}`;
      ...
      try {
        ...
        function onMsg(e) {
          if (e.data && e.data.terminusNav) {
            loadUrl(e.data.terminusNav);
          }
        }
        window.addEventListener('message', onMsg);
        entry._msgCleanup = () => window.removeEventListener('message', onMsg);
      } catch (e) {
